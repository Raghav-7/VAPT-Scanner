  ...
  for (const param of paramsToTest) {
    try {
      ...
      const [trueResp, falseResp, normalResp] = await Promise.all([
        axios.get(trueUrl.toString(), { timeout: 8000, validateStatus: () => true, headers: { 'User-Agent': 'UniversalVAPTScanner/1.0', ...(options.headers || {}) } }),
        axios.get(falseUrl.toString(), { timeout: 8000, validateStatus: () => true, headers: { 'User-Agent': 'UniversalVAPTScanner/1.0', ...(options.headers || {}) } }),
        axios.get(normalUrl.toString(), { timeout: 8000, validateStatus: () => true, headers: { 'User-Agent': 'UniversalVAPTScanner/1.0', ...(options.headers || {}) } }),
      ]);
